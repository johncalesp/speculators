...
export PATH=/workspace/tools/git-package/usr/bin:/workspace/venv-vllm029/bin:$PATH
...
# Exercise the launchers' full Qwen vocabulary default (152064).
unset DRAFT_VOCAB_SIZE
...
if [[ "$1" == visionarena ]]; then
    ...
    export OUTPUT_DIR=/workspace/visionarena-smoke-vllm029 MAX_TURNS=1 REGEN_GPUS=0 EXTRACT_GPUS=0 TRAIN_GPUS=1 SERVER_PORT=8200
else
    export REQUESTS_FILE=/workspace/requests-smoke-vllm029.jsonl OUTPUT_DIR=/workspace/requests-smoke-vllm029 REGEN_GPUS=2 EXTRACT_GPUS=2 TRAIN_GPUS=3 SERVER_PORT=8201
    python - <<'PY'
import json
from pathlib import Path
request=json.loads(Path('examples/data/vlm_attribute_request.json').read_text())
Path('/workspace/requests-smoke-vllm029.jsonl').write_text(''.join(json.dumps(request | {'id': f'smoke-{i}'})+'\n' for i in range(12)))
PY
fi
set +e
